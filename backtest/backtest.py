...
def normalize_index_datetime(df: pd.DataFrame) -> pd.DataFrame:
    """确保索引为datetime，避免epoch秒写出成1970年。"""
    if pd.api.types.is_datetime64_any_dtype(df.index):
        return df
    df = df.copy()
    if pd.api.types.is_integer_dtype(df.index) or pd.api.types.is_float_dtype(df.index):
...
bs_point_out = normalize_index_datetime(bs_point)
equity_curve_out = normalize_index_datetime(result["equity_curve"])
trade_log_out = normalize_index_datetime(result["trade_log"])

bs_point_out.to_csv(os.path.join(output_dir, "bs_point.csv"))
equity_curve_out.to_csv(os.path.join(output_dir, "equity_curve.csv"))
trade_log_out.to_csv(os.path.join(output_dir, "trade_log.csv"))
